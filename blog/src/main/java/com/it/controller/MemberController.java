...
public class MemberController extends BaseController {
    ...
    public String Home(@RequestParam(defaultValue = "1", value = "pageNum") Integer pageNum,int memberid, HttpServletRequest request) {
        ...
        ccc.put("memberid", memberid);
        List<Photo> photolist = photoDAO.selectAll(ccc);
        for (Photo photo : photolist) {
            HashMap bbb = new HashMap();
            bbb.put("photoid", photo.getId());
            List<Imgdetails> imgdetailslist = imgdetailsDAO.selectAll(bbb);
    ...
    @RequestMapping("imgdetailsLb")
    public String imgdetailsLb(@RequestParam(defaultValue = "1", value = "pageNum") Integer pageNum,int memberid, HttpServletRequest request) {
        Member mmm = (Member) request.getSession().getAttribute("member");
        Member member = memberDAO.findById(memberid);
        //查看关注人数
        int gzmemberid = memberid;
        ...
        for(Comment comment:commentlist){
            Member member1 = memberDAO.findById(Integer.parseInt(comment.getMemberid()));
            comment.setMember(member1);
        }
        List<Imgdetails> imgdetailslist = imgdetailsDAO.selectAll(map);
        request.setAttribute("imgdetailslist", imgdetailslist);
