...
<div class="fly-home fly-panel" style="background-image: url();">
    ...
    <div class="fly-sns" data-user="">
        ...
        <c:if test="${sessionScope.member ne null&&fn:length(sglist)==0&& sessionScope.member.id eq member.id}">
            <a href="javascript:qd('${sessionScope.member.id}')" class="layui-btn layui-btn-primary fly-imActive"
               data-type="addFriend">签到</a>
        </c:if>
        ...
        <c:if test="${sessionScope.member ne null  && sessionScope.member.id ne member.id }">
            <c:if test="${sessionScope.member ne null &&(member.id ne sessionScope.member.id)&&(isgz eq '0')}">
                <a href="javascript:gz('${sessionScope.member.id}','${member.id}')"
                   class="layui-btn layui-btn-primary fly-imActive" data-type="addFriend">关注</a>
            </c:if>
            <c:if test="${sessionScope.member ne null &&(member.id ne sessionScope.member.id)&&(isgz ne '0')}">
                <a class="layui-btn layui-btn-normal fly-imActive" data-type="chat"
                   href="javascript:qxgz('${sessionScope.member.id}','${member.id}')">已关注</a>
            </c:if>

            <c:if test="${sessionScope.member ne null &&(member.id ne sessionScope.member.id)&&(ispb eq '0')}">
                <a href="javascript:pb('${sessionScope.member.id}','${member.id}')"
                   class="layui-btn layui-btn-primary fly-imActive" data-type="addFriend">屏蔽</a>
            </c:if>
            <c:if test="${sessionScope.member ne null &&(member.id ne sessionScope.member.id)&&(ispb ne '0')}">
                <a class="layui-btn layui-btn-normal fly-imActive" data-type="chat"
                   href="javascript:qxpb('${sessionScope.member.id}','${member.id}')">已屏蔽</a>
            </c:if>
            <a class="layui-btn layui-btn-primary fly-imActive" data-type="addFriend"
               href="javascript:jb('${member.id}')">举报</a>
            <a class="layui-btn layui-btn-primary fly-imActive" data-type="addFriend"
               href="javascript:sx('${sessionScope.member.id}','${member.id}')">私信</a>
        </c:if>
    </div>

</div>

<div class="layui-container" style="border: 0px solid red;width: 100%">
    <div class="layui-row layui-col-space15">
        <div class="layui-col-md4 fly-home-jie">
            <div class="fly-panel">
                <h3 class="fly-panel-title">${member.tname}最近的文章</h3>
                <ul class="jie-row">
                    <c:forEach items="${pageInfo.list}" var="tzinfo">
                        <li>
                            ...
                            <a href="javascript:;" onclick="tzdetails('${tzinfo.id}')"
                               class="jie-title">${tzinfo.title}</a>
                            <i>${tzinfo.savetime}</i>
        ...
        <div class="layui-col-md8 fly-home-jie">
            <div class="fly-panel">
                <h3 class="fly-panel-title">${member.tname}的相册</h3>
                <div class="layui-row" style="padding: 20px;">
                    <c:forEach items="${imgdetailslist}" var="imgdetials">
                            ...
                            <li data-id="111" class="jieda-daan">
                                ...
                                <div class="detail-about detail-about-reply">
                                    <a class="fly-avatar" href="">
                                        <img src="<%=path %>/upload/${comment.member.filename}" alt=" ">
                                    </a>
                                    <div class="fly-detail-user">
                                        <a href="Home?memberid=${comment.member.id}" class="fly-link">
                                            <cite>${comment.member.tname}</cite>
                                            <i class="iconfont icon-renzheng" title="认证信息：XXX"></i>
                                        </a>
                                        <c:if test="${sessionScope.member!=null and sessionScope.member.id==member.id}">
                                            <a href="javascript:;" onclick="del('${comment.id}')"><span style="color:green">删除</span></a>
                                        </c:if>
                                    </div>
                                    <div class="detail-hits">
                                        <span>${comment.savetime }&nbsp;&nbsp;&nbsp;&nbsp;</span>
                                    </div>
                                </div>
...
<script>
    ...
    function checksub(){
        ...
        if(content==""){
            layer.msg("评论内容不能为空",{icon:5,offset:['400px','800px']})
            return false
        }
        if(hiddenmember==""){
        ...
        $.ajax({
            ...
            success:function(res){
                if(res.data==400){
                    layer.msg("不能评论自己的相册",{icon:5,offset:['400px','800px']})
                }else{
                    location.reload(true)
                }
