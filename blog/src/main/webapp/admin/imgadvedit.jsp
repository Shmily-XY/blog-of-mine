...
<html lang="en">
  <head>
    ...
    <script type="text/javascript" src="/blog/layer/layer.js"></script>
...
<div class="well">
    <div id="myTabContent" class="tab-content">
      <div class="tab-pane active in" id="home">
		    <form id="form" action="<%=path %>/admin/imgadvEdit" method="post">
                <input type="hidden" name="id" value="${imgadv.id}">

		        <label>图片</label>
		        <input name='filename' value="${imgadv.filename}" type='text' class="input-xlarge" id='url' />
		        <label></label>
		        <input type='button' value='上传' onClick="up('url')" style="width: 60px;height: 30px;"/>
		        <label></label>
                <label>URL</label>
                <input type="text"  value="${imgadv.url}" name="url" required class="input-xlarge" oninvalid="setCustomValidity('URL不能为空')" oninput="setCustomValidity('');">
		        <label></label>
    ...
    <script type="text/javascript">
    	...
    	function up(tt)
    	{
    			    layer.open({
    			      ...
    			      area: ['450px', '200px'],
    			      content: '/blog/upload.jsp?Result='+tt
    			    });
    	}
    </script>
  </body>
