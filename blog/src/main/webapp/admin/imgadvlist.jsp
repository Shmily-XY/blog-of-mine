            ...
            <div class="row-fluid">
				<div class="btn-toolbar">
					<div class="search-well">
						    <button class="btn btn-primary" onclick="imgadvadd()" type="button"><i class="icon-plus"></i> 焦点图</button>
					</div>
				</div>
...
<div class="well">
    <table class="table">
      ...
      <c:forEach items="${pageInfo.list}" var="imgadv">
        <tr>
          <td style="width: 50%"><img src="./upload/${imgadv.filename}" ></td>
          <td>${imgadv.url}</td>
          <td>
              <a href="<%=path %>/admin/imgadvShow?id=${imgadv.id}"><i class="icon-pencil"></i></a>
              <a href="<%=path %>/admin/imgadvDel?id=${imgadv.id}" ><i class="icon-remove"></i></a>
          </td>
        </tr>
        </c:forEach>
        <tr >
            <td style="font-weight: bold;font-family:楷体;font-weight: bold; color:blue;text-align: right;" colspan="9">
                <c:if test="${fn:length(pageInfo.list) gt 0}">
                    <div id="pagediv" style="border:0px solid red;">
                        <ul>
                            <li>
                                <a href="admin/imgadvList?pageNum=${pageInfo.hasPreviousPage==false?1:pageInfo.prePage}&key=${key}">上一页</a>
                            </li>
                            <c:forEach begin="1" end="${pageInfo.pages}"
                                       varStatus="status">
                                <li class="pagesz ${status.count eq pageInfo.pageNum ?"acvtive":""}">
                                    <a href="admin/imgadvList?pageNum=${status.count}&key=${key}">${status.count}</a>
                                </li>
                            </c:forEach>
                            <li>
                                <a href="admin/imgadvList?pageNum=${pageInfo.hasNextPage==false? pageInfo.pages : pageInfo.nextPage}&key=${key}">下一页</a>
                            </li>
                        </ul>
                    </div>
                </c:if>
