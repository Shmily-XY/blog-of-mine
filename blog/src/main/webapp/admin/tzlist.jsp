...
<div class="well">
    <table class="table">
      ...
      <c:forEach items="${pageInfo.list}" var="tzinfo">
        ...
        <tr>
          ...
          <td>${fn:length(tzinfo.allhtlist) }</td>
            <td>${tzinfo.dznum}</td>
            <td>${tzinfo.shstatus}</td>
          <td>

              <c:if test="${tzinfo.shstatus eq '待审核'}">
                  <a href="admin/shstatusedit?id=${tzinfo.id}&flag=通过">通过</a>
                  <a href="admin/shstatusedit?id=${tzinfo.id}&flag=未通过">未通过</a>
              </c:if>
      ...
      <tr >
          <td style="font-weight: bold;font-family:楷体;font-weight: bold; color:blue;text-align: right;" colspan="11">
