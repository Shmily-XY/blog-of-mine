            ...
            <div class="row-fluid">
				<div class="btn-toolbar">
					<div class="search-well">
						<form action="<%=path %>/admin/musicList" method="post" class="form-inline">
							<input class="input-xlarge" value="${key}" placeholder="名称..." id="key" name="key" type="text">
		                    <button class="btn" type="submit"><i class="icon-search"></i> Go</button>
						    <button class="btn btn-primary" onclick="musicadd()" type="button"><i class="icon-plus"></i> 音乐</button>
						</form>
					</div>
				</div>
...
<div class="well">
    <table class="table">
      ...
      <c:forEach items="${pageInfo.list}" var="music">
        <tr>
          <td>${music.title}</td>
          <td>
              <a href="<%=path %>/admin/musicShow?id=${music.id}"><i class="icon-pencil"></i></a>
              <a href="<%=path %>/admin/musicDel?id=${music.id}" ><i class="icon-remove"></i></a>
          </td>
        </tr>
        </c:forEach>
        <tr >
            <td style="font-weight: bold;font-family:楷体;font-weight: bold; color:blue;text-align: right;" colspan="9">
                <c:if test="${fn:length(pageInfo.list) gt 0}">
                    <div id="pagediv" style="border:0px solid red;">
                        <ul>
                            <li>
                                <a href="admin/musicList?pageNum=${pageInfo.hasPreviousPage==false?1:pageInfo.prePage}&key=${key}">上一页</a>
                            </li>
                            <c:forEach begin="1" end="${pageInfo.pages}"
                                       varStatus="status">
                                <li class="pagesz ${status.count eq pageInfo.pageNum ?"acvtive":""}">
                                    <a href="admin/musicList?pageNum=${status.count}&key=${key}">${status.count}</a>
                                </li>
                            </c:forEach>
                            <li>
                                <a href="admin/musicList?pageNum=${pageInfo.hasNextPage==false? pageInfo.pages : pageInfo.nextPage}&key=${key}">下一页</a>
                            </li>
                        </ul>
                    </div>
                </c:if>
    ...
    <script type="text/javascript">
    	function musicadd(){
			location.href="admin/musicadd.jsp";
        }
        $("[rel=tooltip]").tooltip();
        $(function() {
